...
`example.sh` wraps everything below into one idempotent, fail-fast script:
`rollout` (Stage A) / `train` (Stage B smoke) / `train --full` (the growth-curve
recipe) / `rollout --backend lexmount` (Stage C).
